		...
		*/
		...
		if(res[running]==false)
		{
			if(t[running]<burst_time[running])
			{
				...
				{ 
					printf("%d\t\t%d\t\t%s\t\t\t%p\n",process[j],pc[j],state[j],&process[j+1]);
					      
				}
				/* update pcb value of process before running */
				updateLabel(GTK_LABEL(pcb1),process[running],pc[running],state[running],&process[running]);
					...
					{ 
						  ...
						  printf("%d\t\t%d\t\t%s\t\t\t%p\n",process[i],pc[i],state[i],&process[i]);
						  
					}

					/* update pcb value of process after running */
					updateL(GTK_LABEL(pcb2),process[running],pc[running],state[running],&process[running]);
					sleep(2);
